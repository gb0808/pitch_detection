pub fn bit_reverse_order(arr: &mut [u8]) {
...
mod tests {
    ...
    fn bit_reverse_order_test() {
        let arr = RefCell::new(vec![0, 1, 2, 3, 4, 5, 6, 7]);
        bit_reverse_order(&mut arr.borrow_mut());
        assert_eq!(arr.borrow()[0], 0);
        assert_eq!(arr.borrow()[1], 4);
        assert_eq!(arr.borrow()[2], 2);
        assert_eq!(arr.borrow()[3], 6);
        assert_eq!(arr.borrow()[4], 1);
        assert_eq!(arr.borrow()[5], 5);
        assert_eq!(arr.borrow()[6], 3);
        assert_eq!(arr.borrow()[7], 7);
    }
}